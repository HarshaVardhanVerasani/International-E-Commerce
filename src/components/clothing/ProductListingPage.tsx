...
import { ChevronRight, Close, FilterList } from "@mui/icons-material";
...
const ProductListingPage: React.FC = () => {
  ...
  const handleFilterChange = (
    ...
    setFilters((prevFilters) => {
      const updatedOptions = prevFilters[filterType].map((option) =>
  ...
  useEffect(() => {
    ...
    const selectedBrands = filters.brands
      ...
      .map((brand) => brand.label.toLowerCase());
    ...
    // Filter by fit types
    const selectedFitTypes = filters.fitTypes
      .filter((fitType) => fitType.checked)
      .map((fitType) => fitType.label.toLowerCase());
    if (selectedFitTypes.length > 0) {
      result = result.filter(
        (product) =>
          product.fitType &&
          selectedFitTypes.includes(product.fitType.toLowerCase())
      );
    }

    // Filter by patterns
    const selectedPatterns = filters.patterns
      .filter((pattern) => pattern.checked)
      .map((pattern) => pattern.label.toLowerCase());
    if (selectedPatterns.length > 0) {
      result = result.filter(
        (product) =>
          product.pattern &&
          selectedPatterns.includes(product.pattern.toLowerCase())
      );
    }
  ...
  return (
    ...
    <Box sx={{ display: 'flex', alignItems: 'center', fontSize: '0.875rem', marginBottom: 3, color: 'text.secondary' }}>
      <Typography component="a" href="/" sx={{ '&:hover': { textDecoration: 'underline' } }}>Home</Typography>
      <ChevronRight sx={{ fontSize: 14, marginX: 1 }} />
      <Typography component="a" href="/women-clothing" sx={{ '&:hover': { textDecoration: 'underline' } }}>Women-Clothing</Typography>
      <ChevronRight sx={{ fontSize: 14, marginX: 1 }} />
      <Typography variant="body2" sx={{ fontWeight: 600, color: 'text.primary' }}>Tops</Typography>
    </Box>


    <Typography sx={{ fontWeight: 600, textAlign: 'center', marginBottom: 2, fontSize: { xs: '20px', sm: '30px' } }}>DESIGNER TOPS</Typography>

 
    <Box sx={{ textAlign: 'center', marginBottom: 4 }}>
      <Typography sx={{ color: 'text.secondary', maxWidth: 900, marginX: 'auto', fontSize: { xs: '15px', sm: '20px' } }}>
        Whether you're searching for diaphanous silk blouses from Dolce & Gabbana...
        <Button sx={{ marginLeft: 1, fontWeight: 500, fontSize: { xs: '15px', sm: '20px' } }} color="inherit">
          Read more
        </Button>
      </Typography>
    </Box>
    ...
    <Box sx={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 4, justifyContent: 'center' }}>
      ...
      <Box sx={{ flex: 1 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
          ...
          <Select value={sort} onChange={handleSortChange} sx={{ minWidth: 180 }}>
